...
This can happen when the Auth server is upgraded but the event handler is not
...
to handle new events, it can cause the event handler to miss important events
and drift from the Auth server truth.

The Teleport event handler receives events from the Auth server, serializes them
using JSON and sends them to the upstream service (e.g. Splunk, Datadog, etc).
When the event handler is not up to date with the Auth server, the new events
ids will not be known to the event handler and the protobuf raw cannot be deserialized.
Without the deserialized event, the event handler cannot serialize the event
using JSON and send it to the upstream service.

Currently, the event handler ignores unknown events.

## Proposal

The proposal is to have a slower local path to handle unknown events. The slower
local path consists of having Auth server serving a WASM module that can perform
...
This means that the raw -> unmashaled -> marshaled -> raw path will be the same even if the
message is unknown to the event handler.
...
and serialize it to JSON and return it to the caller that will send it to the upstream service.
...
because it will add a lot of overhead and will add a lot of latency to protobuf->JSON conversion (it's 400x slower than calling a function).

Given that Teleport uses and abuses GoGo Protobuf for json serialization, it's not possible to have a
drop-in replacement for the current JSON serialization using another language that supports WASI.
...
defines the same functions as `wasm_exec.js` file but written in Go (already have a working version of it). This file is required to run WASM modules compiled with GOOS=js and GOARCH=wasm because it defines the external
functions that the WASM module can call to interact with the host (event handler).
