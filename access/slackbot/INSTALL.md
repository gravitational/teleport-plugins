...
For now, just think of the URL you'll use and set it in the Slack App's settings screeen in Features > Interactive Components > Request URL.
...
To start using Teleport Plugins, you will need to Download the binaries from the customer portal. After downloading the binary tarball, run:
...
### Configuration File
Teleport Slack plugin has it's own config file in TOML format. Before starting the plugin, you'll need to generate (or just copy the one below) and edit that config.
...
Then set the plugin callback (where Slack sends it's requests) to an address you like, and provide the TLS certificates for that http server to use. 
...
Assuming that Teleport is running, and you've created the Slack app, the plugin config, and provided all the certificates — you can now urn the plugin and test the workflow!
...
```bash
vm0:~/slackbot sudo ./slackbot start
