GITREF=`git describe --dirty --long --tags --match '*slack*'`
...
helm-version:
	@if ! echo "$${VERSION}" | grep -q "\-dev"; then \
		...
		export CHART_VERSION=$$(git ls-remote --tags https://github.com/gravitational/teleport-plugins | cut -d'/' -f3 | grep -Ev '(alpha|beta|dev|rc)' | sort -rV | head -n1 | cut -d. -f1 | tr -d '^v'); \
	fi; \
	for CHART in teleport-access-slack; do \
		sed -i'.bak' -e "s_^version:\ .*_version: \"$${CHART_VERSION}\"_g" charts/$${CHART}/Chart.yaml || exit 1; \
		sed -i'.bak' -e "s_^appVersion:\ .*_appVersion: \"$${CHART_VERSION}\"_g" charts/$${CHART}/Chart.yaml || exit 1; \
		rm -f charts/$${CHART}/Chart.yaml.bak; \
	done
