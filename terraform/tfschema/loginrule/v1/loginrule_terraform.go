				...
				if !v.Null && !v.Unknown {
					...
					}
					{
						...
						if !ok {
							...
							if !ok {
								diags.Append(attrReadConversionFailureDiag{"LoginRule.metadata.Expires", "github.com/gravitational/teleport-plugins/terraform/tfschema.TimeValue"})
							} else {
								var t *time.Time
								if !v.Null && !v.Unknown {
									c := time.Time(v.Value)
					...
					}
					{
						...
						if !ok {
							...
							if !ok {
								...
								if !ok {
									diags.Append(attrWriteConversionFailureDiag{"LoginRule.metadata.Expires", "github.com/gravitational/teleport-plugins/terraform/tfschema.TimeValue"})
								}
								v.Null = false
							}
							if obj.Expires == nil {
