...
func (r resourceTeleportOIDCConnectorType) GetSchema(ctx context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return tfschema.GenSchemaOIDCConnectorV2(ctx)
}
...
func (r resourceTeleportOIDCConnector) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	oidcConnector := &apitypes.OIDCConnectorV2{}
	diags = tfschema.CopyOIDCConnectorV2FromTerraform(ctx, plan, oidcConnector)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	oidcConnector, ok := oidcConnectorI.(*apitypes.OIDCConnectorV2)
	if !ok {
		resp.Diagnostics.AddError("Error reading OIDCConnector", fmt.Sprintf("Can not convert %T to OIDCConnectorV2", oidcConnectorI))
		return
	}

	diags = tfschema.CopyOIDCConnectorV2ToTerraform(ctx, *oidcConnector, &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r resourceTeleportOIDCConnector) Read(ctx context.Context, req tfsdk.ReadResourceRequest, resp *tfsdk.ReadResourceResponse) {
	...
	oidcConnector := oidcConnectorI.(*apitypes.OIDCConnectorV2)
	diags = tfschema.CopyOIDCConnectorV2ToTerraform(ctx, *oidcConnector, &state)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r resourceTeleportOIDCConnector) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error deleting OIDCConnectorV2", err.Error())
		return
	}
