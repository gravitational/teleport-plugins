...
func (tctl Tctl) Sign(ctx context.Context, username string, ttl time.Duration, outPath string) error {
	log := logger.Get(ctx)
...
func (tctl Tctl) SignToString(ctx context.Context, username string, ttl time.Duration) (string, error) {
	...
	if err := tctl.Sign(ctx, username, ttl, identityFile.Name()); err != nil {
		return "", trace.NewAggregate(
			trace.Wrap(err),
			os.Remove(identityFile.Name()),
		)
