...
func (s *IdentitySuite) SetupSuite() {
	...
	_, err := bootstrap.AddRole(TestIdentityUsername, types.RoleSpecV5{
...
func (s *IdentitySuite) createIdentity(ttl time.Duration) *authv10.Identity {
	t := s.T()
	t.Helper()
...
func (s *IdentitySuite) waitSecretCreation(identity *authv10.Identity) *corev1.Secret {
	t := s.T()
	t.Helper()
...
func (s *IdentitySuite) waitSecretUpdate(identity *authv10.Identity, prevValue []byte) *corev1.Secret {
	t := s.T()
	t.Helper()
