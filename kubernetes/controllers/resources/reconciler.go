...
func addOriginLabel(ctx context.Context, kClient kclient.Client, obj kclient.Object) error {
	kObjLabels := obj.GetLabels()
	if kObjLabels == nil {
		kObjLabels = make(map[string]string)
	}

	kObjLabels[types.OriginLabel] = types.OriginCloud /* types.OriginKubernetes */
	obj.SetLabels(kObjLabels)

	return trace.Wrap(kClient.Update(ctx, obj))
}
