...
func checkImpersonationPermissions(ctx context.Context, sarClient authztypes.SelfSubjectAccessReviewInterface) error {
	...
	for _, resource := range []string{"users", "groups", "serviceaccounts"} {
		...
		if !resp.Status.Allowed {
			return trace.AccessDenied("proxy can't impersonate kubernetes %s at the cluster level; please make sure that proxy has all the necessary permissions: https://goteleport.com/teleport/docs/kubernetes_ssh/#impersonation", resource)
		}
	}
	return nil
}
